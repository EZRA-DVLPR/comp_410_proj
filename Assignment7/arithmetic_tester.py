...
def makeTest(bound):
    ...
    #make the sub expressions with bound - 1
    if bound > 0:
        for LHS in makeTest(bound - 1):
            for RHS in makeTest(bound - 1):
                yield Plus(LHS, RHS)
                yield Minus(LHS, RHS)
                yield Mult(LHS, RHS)
                yield Negate(LHS)
...
def makeTestWithNums(bound, input_list):
    ...
    #yield Number(NUMBER) for each NUMBER in input_list
    for num in input_list:
        yield Number(num)
    
    #make the sub expressions with bound - 1
    if bound > 0:
        for LHS in makeTestWithNums(bound - 1, input_list):
            for RHS in makeTestWithNums(bound - 1, input_list):
                yield Plus(LHS, RHS)
                yield Minus(LHS, RHS)
                yield Mult(LHS, RHS)
                yield Negate(LHS)
