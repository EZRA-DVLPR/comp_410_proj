...
def solve(goals, literals):
    ...
    if isinstance(goals, Nil):
        #print(literals.__dict__)
        return literals

    elif (isinstance(goals.head, And)):
        #left side of And goes to head, tail becomes the right side and the tail beforehand
        return solve(Cons(goals.head.left, Cons(goals.head.right, goals.tail)), literals)

    elif (isinstance(goals.head, Or)):
        #if LHS gives None, then return RHS. Otherwise, return LHS
        LHS = solve(Cons(goals.head.left, goals.tail), literals)
        if LHS is None:
            return solve(Cons(goals.head.right, goals.tail), literals)
        else:
            return LHS
        
    elif (isinstance(goals.head, Literal)):
        #if literal is not added, return None. Otherwise update goals based on what is next
        ...
        print(goals, goals.head, goals.tail)
        return
