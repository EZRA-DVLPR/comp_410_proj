...
%Sums the inclusive range from [Low, High] and then returns output Sum
sumAll(High, High, High).
sumAll(Low, High, Sum) :-
    Low < High,
    NewLow is Low + 1,
    sumAll(NewLow, High, NewSum),
    Sum is Low + NewSum.
...
%if the the course A is the direct prereq of course B return true.
%otherwise recursively find a path from course A to course B if possible.
prereq(CourseA, CourseB) :-
    directPrereq(CourseA, CourseB).
prereq(CourseA, CourseB) :-
