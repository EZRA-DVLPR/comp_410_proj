...
%base case: there are no more `or` or `and` statements in the expr
eval(true, true).
eval(false, false).

%evaluate the LHS and RHS of the `or` or `and` statement within the given expr
eval(and(LHS, RHS), Result) :-
    eval(LHS, ResultL),
    eval(RHS, ResultR),
    and(ResultL, ResultR, Result).

eval(or(LHS, RHS), Result) :-
    eval(LHS, ResultL),
    eval(RHS, ResultR),
    or(ResultL, ResultR, Result).
